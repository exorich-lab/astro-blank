...
- После `serp-sitemaps` обязательно валидировать гипотезы через Bing keyword stats командой `make serp-plan-validate-expansion RUN_DIR=/path/to/run SEED_PLAN=plan.json COUNTRY=au LANGUAGE=en-AU MARKET_LABEL=australia`. Этот слой должен сам создавать разные поисковые варианты по каждой гипотезе, проверять exact/broad impressions, отбрасывать слишком общие запросы вроде одного слова `bonus`, и записывать результат в `validatedSitemapExpansion.clusters`, `seeds` и `clusterRules` нового seed-plan.
